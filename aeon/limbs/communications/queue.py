"""SQLite-backed persistent queue for unsent outbound emails."""
...
class QueuedEmail:
    ...
    text_body: str
    html_body: str
    category: str  # "research_update", "urgent_alert", "digest"
    retry_count: int
...
class EmailQueue:
    ...
    with exponential backoff by the email sending loop.
    """
    ...
    CREATE TABLE IF NOT EXISTS pending_emails (
        ...
        category       TEXT    NOT NULL DEFAULT 'research_update',
        retry_count    INTEGER NOT NULL DEFAULT 0,
        next_retry_at  TEXT    NOT NULL,
        created_at     TEXT    NOT NULL
    );
    CREATE INDEX IF NOT EXISTS idx_next_retry ON pending_emails(next_retry_at);
    CREATE INDEX IF NOT EXISTS idx_category ON pending_emails(category);
    """
        ...
        try:
            cursor = conn.execute(
                ...
                INSERT INTO pending_emails (
                    ...
                    category, retry_count, next_retry_at, created_at
                ) VALUES (?, ?, ?, ?, ?, 0, ?, ?)
                RETURNING id
                """,
                args,
