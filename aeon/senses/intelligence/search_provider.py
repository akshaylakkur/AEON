"""Abstract search provider interface for the AEON intelligence layer.

Defines the ``SearchProvider`` ABC and a ``SearchResult`` dataclass that
all search providers must use. Also provides a factory function
``create_search_provider()`` that returns the best available provider.
"""
...
class SearchProvider(ABC):
    ...
    All search providers must implement ``search()`` and return a list of
    ``SearchResult``. The interface is intentionally simple so that
    DuckDuckGo and SerpAPI are interchangeable.
    """
...
def create_search_provider() -> SearchProvider | None:
    ...
    # Also check the HedgeFundConfig if available
    try:
        from aeon.core.config import get_config
        cfg = get_config()
        if not serpapi_key:
            serpapi_key = cfg.serpapi_key or ""
    except Exception:
        pass
    ...
    logger.info("Using DuckDuckGo search provider (no API key required).")
