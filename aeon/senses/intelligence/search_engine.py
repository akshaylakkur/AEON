"""Web search meta-engine with caching and rate limiting.

Supports SerpAPI and Brave Search backends. Routes queries to the
appropriate provider based on configuration. Uses an in-memory cache
to minimize API spend.

For most use cases, prefer ``create_search_provider()`` from
``search_provider.py`` which returns a simpler ``SearchProvider``
interface. This module exists for direct SerpAPI/Brave access with
caching.
"""
...
class SearchEngine:
    ...
    async def search(
        ...
        if not self._supports(provider):
            logger.warning("Search provider '%s' is not configured (missing API key).", provider)
            return []
    ...
    async def _search_serpapi(self, query: str, num_results: int) -> list[SearchResult]:
        if not self._serpapi_key:
            logger.warning("SerpAPI key is not configured.")
            return []
