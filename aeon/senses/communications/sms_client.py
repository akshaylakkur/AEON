...
OPTIONAL -- requires Twilio credentials to function. If not configured,
all methods return graceful error dicts.
...
class SmsClient(BaseConnector):
    ...
    OPTIONAL -- requires Twilio credentials. If not configured, all
    methods return graceful error dicts.
    """
    ...
    def is_configured(self) -> bool:
        """Return True if Twilio credentials are available."""
    ...
    async def connect(self) -> None:
        ...
        if not self.is_configured:
            logger.info(
                "SmsClient: Twilio not configured. "
                "Set AEON_TWILIO_ACCOUNT_SID and AEON_TWILIO_AUTH_TOKEN."
            )
            return
