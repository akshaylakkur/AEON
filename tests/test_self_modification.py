"""Comprehensive pytest tests for aeon.metamind self-modification components."""
...
from aeon.core.events import CodeModified, DependencyInstalled, ModificationFailed
from aeon.core.state_machine import State, StateMachine
from aeon.ledger.cost_tracker import CostCategory, CostTracker
from aeon.ledger.master_wallet import MasterWallet
from aeon.metamind import (
    CodeIntrospector,
    CodePatch,
    DependencyManager,
    DiffParseError,
...
def tmp_project(tmp_path: Path) -> Path:
    """Create a temporary project root with aeon/ tree."""
    aeon_pkg = tmp_path / "aeon"
    aeon_pkg.mkdir()
    (aeon_pkg / "__init__.py").write_text("")
    core = aeon_pkg / "core"
    core.mkdir()
    (core / "__init__.py").write_text("")
    (core / "constants.py").write_text("DEFAULT_RESEARCH_BUDGET_DAILY = 1.00\n")
    senses = aeon_pkg / "senses"
    senses.mkdir()
    ...
    import shutil
    real_templates = Path(__file__).resolve().parents[1] / "aeon" / "metamind" / "templates"
    if real_templates.exists():
        dest = aeon_pkg / "metamind" / "templates"
...
def module_generator(mock_llm: MagicMock, tmp_project: Path) -> ModuleGenerator:
    return ModuleGenerator(
        llm=mock_llm,
        template_dir=tmp_project / "aeon" / "metamind" / "templates",
...
def sme(
    ...
    # Seed wallet with a nominal balance for testing
    wallet.credit(50.0, "seed")
...
def test_build_source_map(introspector: CodeIntrospector) -> None:
    sm = introspector.build_source_map("aeon")
    assert "aeon.core.constants" in sm.modules
    assert "aeon.senses.market_data" in sm.modules


def test_locate_function(introspector: CodeIntrospector) -> None:
    introspector.build_source_map("aeon")
...
def test_describe_module(introspector: CodeIntrospector) -> None:
    introspector.build_source_map("aeon")
    info = introspector.describe_module("aeon.senses.market_data")
...
def test_extract_dependencies(introspector: CodeIntrospector) -> None:
    introspector.build_source_map("aeon")
    deps = introspector.extract_dependencies("aeon.senses.market_data")
    assert "httpx" in deps


def test_compute_complexity(introspector: CodeIntrospector) -> None:
    introspector.build_source_map("aeon")
    mc = introspector.compute_complexity("aeon.senses.market_data")
    assert mc.total >= 1
...
def test_generate_from_spec(mock_llm: MagicMock, tmp_project: Path) -> None:
    ...
    tpl_dir = tmp_project / "aeon" / "metamind" / "templates"
...
def test_apply_patch_simple_addition(
    ...
    target = tmp_project / "aeon" / "senses" / "market_data.py"
    original = target.read_text(encoding="utf-8")
    diff = (
        "--- a/aeon/senses/market_data.py\n"
        "+++ b/aeon/senses/market_data.py\n"
        "@@ -5,3 +5,5 @@\n"
        " class MarketDataFeed:\n"
        "     async def fetch(self, symbol: str) -> dict[str, Any]:\n"
        "         return {'symbol': symbol}\n"
...
def test_scan_module_for_missing_deps(dependency_manager: DependencyManager, tmp_project: Path) -> None:
    mod = tmp_project / "aeon" / "senses" / "market_data.py"
...
async def test_sme_protected_path_blocked(sme: SelfModificationEngine, tmp_project: Path) -> None:
    sme.wallet.credit(1000.0, "test_fund")
    target = tmp_project / "aeon" / "core" / "constants.py"
    diff = (
        "--- a/aeon/core/constants.py\n"
        "+++ b/aeon/core/constants.py\n"
        "@@ -1 +1 @@\n"
        "-DEFAULT_RESEARCH_BUDGET_DAILY = 1.00\n"
        "+DEFAULT_RESEARCH_BUDGET_DAILY = 2.00\n"
    )
    patch = CodePatch(
        patch_id="sme_p3",
        target_file=target,
...
async def test_sme_introspect_module(sme: SelfModificationEngine) -> None:
    info = await sme.introspect_module("aeon.senses.market_data")
    assert info.classes[0].name == "MarketDataFeed"
...
async def test_sme_hibernation_advisory(
    ...
    assert sm.current_state in (State.SLEEPING, State.RESEARCHING)
...
async def test_sme_compute_complexity(sme: SelfModificationEngine) -> None:
    mc = sme.compute_complexity("aeon.senses.market_data")
    assert mc.total >= 1
...
def test_self_modification_error_is_exception() -> None:
    from aeon.metamind.code_introspector import SelfModificationError
