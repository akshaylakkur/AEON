...
from aeon.analytics.alpha_engine import AlphaEngine
from aeon.analytics.backtester import Backtester
from aeon.analytics.dataclasses import AlphaSignal, BacktestResult, RevenueOpportunity, RiskAssessment
from aeon.analytics.revenue_engine import RevenueEngine
from aeon.analytics.risk_management import RiskManager
...
class TestRiskManager:
    ...
    def test_recommended_position_size(self):
        rm = RiskManager()
        assessment = rm.recommended_position_size(portfolio_value=100.0, edge=0.6)
        assert isinstance(assessment, RiskAssessment)
        assert assessment.position_cap == 0.05

    def test_recommended_position_size_zero_portfolio(self):
        rm = RiskManager()
        assessment = rm.recommended_position_size(portfolio_value=0.0, edge=0.6)
        assert assessment.approved is False
        assert assessment.position_size_pct == 0.0

    def test_max_position_size_backward_compat(self):
        rm = RiskManager()
