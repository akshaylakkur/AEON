"""Comprehensive pytest suite for aeon/reflexes."""
...
from aeon.core.events import EmergencyLiquidate, Hibernate
from aeon.reflexes.api_health import APIHealthMonitor
from aeon.reflexes.circuit_breakers import CircuitBreakers
from aeon.reflexes.dataclasses import ApiDown, ApiRecovered, LiquidationOrder, PositionSize
...
class TestPositionSizer:
    def test_kelly_within_cap(self) -> None:
        sizer = PositionSizer()
        ...
        # Kelly = 0.01 / 1.0 = 0.01, cap = 0.05 -> fraction = 0.01
        # quantity = 1000 * 0.01 = 10
        assert result.quantity == Decimal("10")
        assert result.max_loss == Decimal("10")
    ...
    def test_kelly_capped_at_max(self) -> None:
        ...
        # Kelly = 0.10, cap = 0.05 -> fraction = 0.05
        # quantity = 1000 * 0.05 = 50
        assert result.quantity == Decimal("50")

    def test_tier_param_ignored(self) -> None:
        sizer = PositionSizer()
        result_t0 = sizer.calculate_position_size(
            balance=Decimal("1000"),
            edge=Decimal("0.05"),
            odds=Decimal("1.0"),
            tier=0,
        )
        result_t3 = sizer.calculate_position_size(
            balance=Decimal("1000"),
            edge=Decimal("0.05"),
            odds=Decimal("1.0"),
            tier=3,
        )
        # tier parameter is ignored in research-only mode
        assert result_t0.quantity == result_t3.quantity
    ...
    def test_small_edge(self) -> None:
        ...
        # Kelly = 0.02, cap = 0.05 -> fraction = 0.02
        # quantity = 100 * 0.02 = 2
        assert result.quantity == Decimal("2")
        assert result.max_loss == Decimal("2")
