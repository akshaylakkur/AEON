...
async def test_binance_fetch_not_connected() -> None:
    ...
    result = await conn.fetch_data({"endpoint": "/test"})
...
@pytest.mark.asyncio
async def test_twitter_not_configured_connect(twitter_connector: TwitterSentimentConnector) -> None:
    """Without bearer token, connect() is a no-op and connected stays False."""
    assert not twitter_connector.is_configured
    await twitter_connector.connect()
    assert not twitter_connector.connected


@pytest.mark.asyncio
async def test_twitter_configured_connect_disconnect() -> None:
    """With a bearer token, connect() and disconnect() work normally."""
    conn = TwitterSentimentConnector(bearer_token="test_token_123")
...
async def test_twitter_cost_and_tier(twitter_connector: TwitterSentimentConnector) -> None:
    ...
    assert cost == {"monthly": 0.0, "daily": 0.0}
    # Without a bearer token, is_available returns False
    assert not twitter_connector.is_available(0)
    assert not twitter_connector.is_available(1)


@pytest.mark.asyncio
async def test_twitter_cost_and_tier_configured() -> None:
    conn = TwitterSentimentConnector(bearer_token="test_token")
    cost = conn.get_subscription_cost()
    assert cost == {"monthly": 0.0, "daily": 0.0}
    assert conn.is_available(0)


@pytest.mark.asyncio
async def test_twitter_search_tweets_not_configured(twitter_connector: TwitterSentimentConnector) -> None:
    """search_tweets returns error dict when not configured."""
    result = await twitter_connector.search_tweets("bitcoin", max_results=5)
    assert "error" in result
    assert result["configured"] is False


@pytest.mark.asyncio
async def test_twitter_fetch_data_not_configured(twitter_connector: TwitterSentimentConnector) -> None:
    """fetch_data returns error dict when not configured."""
