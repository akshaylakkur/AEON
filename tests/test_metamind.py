"""Comprehensive pytest tests for aeon.metamind."""
...
from aeon.metamind.dataclasses import SafetyRating as _SafetyRating
...
def temp_source_dir(tmp_path: Path) -> Path:
    ...
    core = pkg / "core"
    core.mkdir()
    (core / "__init__.py").write_text("from aeon.core.events import EventBus\n")
...
def engine(
    ...
    return AdaptionEngine(
        ...
        gate=gate,
        journal=journal,
        config=AdaptionConfig(cooldown_minutes=0.0, target_module="aeon"),
    )
...
def test_analyze_source_tree(analyzer: SelfAnalyzer) -> None:
    ...
    assert "aeon.core.events" in sm.modules
    assert "aeon.security.sandbox" in sm.modules


def test_build_dependency_graph(analyzer: SelfAnalyzer) -> None:
    graph = analyzer.build_dependency_graph()
    assert "aeon.core.events" in graph.get("aeon.core.__init__", [])
...
async def test_emit_event(engine: AdaptionEngine, caplog: pytest.LogCaptureFixture) -> None:
    import logging
    logger = logging.getLogger("aeon.metamind.adaption_engine")
