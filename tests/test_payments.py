...
@pytest.mark.asyncio
async def test_stripe_payments_health_check_skeleton(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("STRIPE_SECRET_KEY", raising=False)
    limb = StripePaymentsLimb()
    health = await limb.health_check()
...
@pytest.mark.asyncio
async def test_crypto_onramp_health_check_skeleton(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("MOONPAY_API_KEY", raising=False)
    monkeypatch.delenv("MOONPAY_SECRET_KEY", raising=False)
    monkeypatch.delenv("STRIPE_CRYPTO_SECRET_KEY", raising=False)
